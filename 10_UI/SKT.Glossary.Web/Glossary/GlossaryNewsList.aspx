<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPages/GlossarySearch.master" AutoEventWireup="true"
    CodeBehind="GlossaryNewsList.aspx.cs" Inherits="SKT.Glossary.Web.Glossary.GlossaryNewsList" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder_Common_Head" runat="server">
    <script type="text/javascript">
        var DefaultSearchText = "우리 모임의 게시글을 검색해 보세요.";
        $(document).ready(function () {

            var iTotCnt = "<%=iTotalCountGatheringList%>";
            var SearchTxt = $("#<%= hid_SearchKeyword.ClientID%>").val();

            $("#txt_SearchKeyword").val(DefaultSearchText);

            if (SearchTxt.length > 0 && parseInt(iTotCnt) == 0) {
                $("#no_data").attr("style", "display:;text-align:center; padding-top:30px;");
                //$("#search_no_data").attr("style", "display:");

                $("#list_area").attr("style", "display:none;");
                $("#directory_user").attr("style", "display:none;");

                $("#<%= hid_SearchKeyword.ClientID%>").val("");
            }
        ...
        function inputLengthCheck(inputText) {
        ...
        function setTag(Table, Id) {
            var tempTag = "";
            var GatheringYN = "<%=this.GatheringYN %>";
            for (var i = 0; i < Table.length; i++) {
                ...
                if (GatheringYN != "Y")
                    tempTag += '<li style=\" width:270px;\"><a href=javascript:fnGlossaryList(\'' + encodeURIComponent(encodeURIComponent(LinkTagTitle)) + '\',\'\') class="' + tagCss + '" >' + inputLengthCheck(TagTitle) + '(' + Table[i].ShowCount + ')</a></li>';
                else 
                {
                    ...
                    {
                        tempTag += '<li style=\" width:270px;\"><a href=javascript:fnGlossaryList(\'\',\'CreateDate\') class="' + tagCss + '" >' + inputLengthCheck(TagTitle) + '(' + Table[i].ShowCount + ')</a></li>';
                    }
                }
        ...
        function GetMoreTagOpen() {
            ...
            if ($("#GetMoreBtn").attr("class") == "btn") {
                ...
                if ("<%=GatheringYN%>" == "Y")
                    $("#tag_box").attr("style", "height:190px;overflow-y:auto; ");
                else 
...
<script type="text/javascript">
    function fnFileSearch() {
        ...
        if ($('#txt_SearchKeyword').val() == DefaultSearchText || $('#txt_SearchKeyword').val().replace(/^\s+|\s+$/g, '') == "") {
            ...
            {
                $("#<%= hid_SearchKeyword.ClientID%>").val($("#txt_SearchKeyword").val());
                $("#<%= btnSearchKeyword.ClientID%>").click();
            }
        }
	}
</script>
	...
	<div id="contents">
		...
		<p class="btn_top">
            <%if (GatheringYN == "Y") {  }  else { %>
            <a href="/Glossary/GlossaryWriteNew.aspx?GatheringYN=<%=GatheringYN%>&GatheringID=<%=GatheringID%>"><img src="/common/images/btn/write<%if(GatheringYN=="Y"){ %>_gathering<%}%>.png" alt="글쓰기" /></a></p>
            <%} %>
        ...
        <div id="article" class="Glossary_list" <%if (GatheringYN != "Y") { Response.Write("style = \"padding-top:30px;\");"); }%>>
           ...
           <%-- <div style="width:100%;overflow:hidden; text-align:right; padding-bottom:3px;">
			...
			<div id="list_area">
                ...
                <div id="tag_box" class="tag_box" style="display:none;overflow-x:scroll;">
                 <ul></ul>
                </div>
                ...
                <% if (TagTitle.Trim().Length > 0)
                    {
                        //if (iTotalCountList > 0)
                        //    Response.Write(TagTitle + "(" + iTotalCountList.ToString() + ")"); 
                        //else
                            Response.Write(TagTitle);

                    }
                    else
                    {
                        string SearchSortTagTitle = "";

                        if (SearchSort.ToUpper() == "HITS")
                        {
                            SearchSortTagTitle = "인기지식";
                        }
                        else if (SearchSort.ToUpper() == "LIKE")
                        {
                            SearchSortTagTitle = "추천지식";
                        }
                        else
                        {
                            ...
                            {
                                SearchSortTagTitle = "최신지식";
                            }
                        }
                        Response.Write(SearchSortTagTitle);
                    }
                %>
                </strong></span>
                </div>
                <div style=" float:right; width:10%; overflow:hidden; padding-right:10px; padding-bottom:10px;">
                    <a id="GetMoreBtn" href="javascript:GetMoreTagOpen();" class="btn"><img src="/Common/images/btn/btn_tag_open.png" id="ImgTag" border="0" alt=""/></a>
                </div>
                <!-- 모임목록정보 -->
				<table class="listTable">
					<colgroup><col width="7%" /><col width="*" /><col width="11%" /><col width="20%" /><col width="8%" /><col width="6%" /></colgroup>
					<thead>
					<tr>
						<th>No</th>
						<th>제목</th>
			...
			<!--/리스트-->
			<!--랭킹-->
			<div class="raking_list" <%if (GatheringYN == "Y")
                                           { %>style="display:none;" <% } %>>
				<dl>
					<dt>인기조회순</dt>
            ...
            <div id="directory_user" <%if (GatheringYN != "Y")
                    ...
                    <p id="p_gatheringsetting" runat="server" visible="false"><a href="javascript: GI_viewDivShow();"><img src="/common/images/icon/setting.png" alt="" title="폴더사용자 관리 바로가기" /></a></p>
                    <ul>
...
<asp:Content ID="Content3" ContentPlaceHolderID="ContentPlaceHolder_Common_Footer"
    ...
    <script type="text/javascript">
        ...
        function fnView(ItemID, HistoryYN) {
            ...
            var LinkTagTitle = TagTitle.replace(/&/g, "$");
            LinkTagTitle = encodeURIComponent(LinkTagTitle);

            if(result){
                var url = "/Glossary/GlossaryView.aspx?ItemID=" + ItemID + "&SearchSort=<%=SearchSort%>&PageNum=<%=pager.CurrentIndex%>&TagTitle=" + LinkTagTitle;
                location.href = url;
            }
        }
