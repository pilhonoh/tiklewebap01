...
function DetailView(weeklyID, currentUserID, viewLevel, weeklyUserID, commWeeklyFlag, deptCode, deptName) {
    ...
    if (!(weeklyID == null || weeklyID == "null" || weeklyID == 0)) {
        ...
        //alert(base64_encode(currentUserID));

        //2015.07.10 zz17779 : 공용위클리인 경우 호출 url 변경
        if (tmpCmmWeeklyFlag == "Y") {

            url = "/Weekly/WeeklyListTeam.aspx/GetCommonWeekly";
        }
        else
            url = "/Weekly/WeeklyListTeam.aspx/GetWeekly";
                ...
                */
                ...
                if (permission) {
                    ...
                    /*
                        ...
                        Desc : 메모와 첨부파일도 권한을 체크해야 함.(3자테스트)  
                    */
                    ////메모와 첨부파일도 권한을 체크해야 함.
                    if (viewLevel != '1' && contextTable.length > 0
                        && contextTable[0].PermissionYN == 'Y'
                        && (contextTable[0].PermissionsUserID == null || contextTable[0].PermissionsUserID != currentUserID)
                        && contextTable[0].UserID != currentUserID) 
                    {
                    }
                    else
                    {
        ...
        $.ajax({
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: "/Weekly/WeeklyListTeam.aspx/GetExistsDept",
            data: "{'UserID':'" + currentUserID + "'}",
            dataType: "json",
            async:false,
            success: function (data) {
               
                if (data.d.Table.length > 0) {
                    ExistsDeptCode = true;
                }
                else {
                    ExistsDeptCode = false;
                }
            },
            error: function (result) {
                if (result.status != 0)
...
function BindingView(table, currentUserID, viewLevel, weeklyUserID, deptCode, deptName) {
    ...
    if (viewLevel != '1' && table.length > 0
        && table[0].PermissionYN == 'Y'
        && (table[0].PermissionsUserID == null || table[0].PermissionsUserID != currentUserID)
        && weeklyUserID != currentUserID) {
    ...
    if (table.length > 0 && table[0].PermissionYN != 'Y') {
        ...
        if (tempYN == 'Y' && table[0].UserID != currentUserID) {
            ...
            if (currentUserID == weeklyUserID) {
                //팀원이거나 예외부서 사용자가 아닐경우
                //P097010 BACKUP1
                if (ExistsDeptCode == false && viewLevel == "4") {
                    //@@dHtml += '      <a href="javascript:///" class="btnB" onclick="WriteWeekly()"><span class="memo">Weekly 작성하기</span></a>';
                }
            }
        ...
        if (table[0].WeeklyID == "0" || table[0].WeeklyID == undefined || table[0].WeeklyID == "undefined") {
            ...
            */
            ...
            //dHtml += '<p class="text" data-weeklyID=' + table[0].WeeklyID + '><iframe class="StandaloneView' + table[0].WeeklyID + '" src="WeeklyIframe_TeamMember.aspx?WeeklyID=' + table[0].WeeklyID + '"  scrolling="no" frameborder="0" style="width:100%"><div class="view_ct_area" style="padding-top:10px;"><div class="view_ct" style="margin:0;">' + table[0].Contents + '</div></div></iframe>'
            dHtml += '<p class="text" data-weeklyID=' + table[0].WeeklyID + '><iframe class="StandaloneView' + table[0].WeeklyID + '" src="WeeklyIframe_TeamMember.aspx?WeeklyID=' + tmpWeeklyID + '"  scrolling="no" frameborder="0" style="width:100%"><div class="view_ct_area" style="padding-top:10px;"><div class="view_ct" style="margin:0;">' + table[0].Contents + '</div></div></iframe>'
        }
        dHtml += '</p>';
        ...
        if (viewLevel == 3) { //팀장인경우.
            ...
            if (currentUserID == table[0].UserID)    // 자기 자신의 위클리에만 편집하기 버튼이 보입니다 
            {
                ...
                if (tmpCmmWeeklyFlag == "N") //N이면 기존 편집 , y이면 공통편집
                    dHtml += '<a href="javascript:///" class="btnB" onclick="WriteWeekly();" id="erpmemoview" style="margin-left:5px;"><span class="memo">편집하기</span></a>';
                else {
                ...
                if (tmpCmmWeeklyFlag == "N") { //N이면 기존 편집 , y이면 공통편집
                    ...
                    if (table[0].ViewLevel == "4") {
                        ...
                        //NT dHtml += '<a href="javascript:///" class="btnB" onclick="layer_open(\'layer2\');return false;" id="coachy"><span class="memo">Weekly Note</span></a>';

                        if (table[0].MemoWriterID == "") {    // 메모가 있다면 색변경
                            //NT dHtml += '<a href="javascript:///" class="btnB m_l5" id="erpmemoview" onclick="MemoViewEHR(\'' + currentUserID + '\',\'' + weeklyUserID + '\');return false;"><span class="memo-list">Note 보기</span></a>';
                        } else {
                            //NT dHtml += '<a href="javascript:///" class="btnB m_l5 completion" id="erpmemoview" onclick="MemoViewEHR(\'' + currentUserID + '\',\'' + weeklyUserID + '\');return false;"><span class="memo-list">Note 보기</span></a>';
                        }
            ...
            if (tmpCmmWeeklyFlag == "N") {
                ...
                if (currentUserID == table[0].UserID)    // 자기 자신의 위클리에만 편집하기 버튼이 보입니다
                {
                    ...
                    if (table[0].WeeklyID == "0" || table[0].WeeklyID == undefined || table[0].WeeklyID == "undefined")
                        ...
                        */
                        ...
                        //dHtml += '<input type="checkbox" id="chkContentDel" onclick="fnWeeklyDelete(\'' + table[0].WeeklyID + '\' );"  style="cursor:pointer;"/><label style="font-size:13px;color:red;vertical-align:-1px;margin-left:5px;">본문 삭제하기23</label>';
                    }
                }
                ...
                if (viewLevel == 3 || viewLevel == 4) {
                    //2015.09.09 zz1777: 3주삭제 처리 안하기로 요청(전소영M) : 위클리삭제는 팀장과 작성자만 권한 존재.
                    if (table[0].WeeklyID == "0" || table[0].WeeklyID == undefined || table[0].WeeklyID == "undefined")
                        dHtml += '<input type="checkbox" id="chkContentDel" onclick="fnWeeklyDelete(0);"  style="cursor:pointer;"/><label style="font-size:13px;color:red;vertical-align:-1px;margin-left:5px;">본문 삭제하기10</label>';
                    else
                        dHtml += '<input type="checkbox" id="chkContentDel" onclick="fnWeeklyDelete(\'' + table[0].WeeklyID + '\' );"  style="cursor:pointer;"/><label style="font-size:13px;color:red;vertical-align:-1px;margin-left:5px;">본문 삭제하기</label>';
                }
            }
                ...
                if (table[0].SameTeamYN == "Y" && table[0].ViewLevel != 3 || currentUserID == table[0].UserID) {
                    ...
                    */
                    ...
                    if (deptCode == "undefined" || deptCode == undefined){
                        ...
                        dHtml += '<a href="javascript:///" class="btnB" onclick="WriteWeekly(\'' + "N" + '\',\'' + deptCode + '\', \'' + deptName + '\');" id="erpmemoview" style="margin-left:5px;"><span class="memo">편집하기</span></a>';
                    }
                    
                    
                    //dHtml += '<a href="javascript:///" class="btnB" onclick="WriteWeekly();" id="erpmemoview" style="margin-left:5px;"><span class="memo">편집하기_여기다</span></a>';
            ...
            if (tmpCmmWeeklyFlag == "Y") {
                ...
                */                
                if (currentUserID == weeklyUserID) {
                    ...
                    //팀원이거나 예외부서 사용자가 아닐경우
                    if (ExistsDeptCode == false && viewLevel == "4") {
                        //@@dHtml += '      <a href="javascript:///" class="btnB" onclick="WriteWeekly(\'' + "N" + '\',\'' + deptCode + '\', \'' + deptName + '\')"><span class="memo">Weekly 작성하기</span></a>';
                    }
                    //dHtml += '      <a href="javascript:///" class="btnB" onclick="WriteWeekly()"><span class="memo">Weekly 작성하기</span></a>';
                }
            }
...
function CommentCtlCreate(weeklyID) {
    ...
    if (!(weeklyID == 0 || weeklyID == null || weeklyID == "" || weeklyID == "null")) {
        ...
        /*
           ...
           Desc : 파라미터 암호화
        */
        var tmpWeeklyID = EncryptParam(weeklyID);

        // 댓글 리스트
        $.ajax({
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: "/Weekly/WeeklyCommentControl.aspx?WeeklyID=" + tmpWeeklyID + "&FirstUserID=" + userID,
            data: "",
            dataType: "html",
            async: false,
            success: function (data) {
...
function AttachArea(weeklyID) {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "/Weekly/WeeklyListTeam.aspx/GetAttachInfo",
        data: JSON.stringify(para),
        dataType: "json",
        async : false,
        success: function (data) {
...
function fnGetUser2(WeeklyUserID) {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "/Weekly/WeeklyListTeam.aspx/GetUserInfo",
